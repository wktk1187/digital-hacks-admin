...
create or replace function public.update_meeting_stats(
  p_email text,
  p_category text default 'teacher',
...
begin
  insert into stats_all(key_year, key_month, key_date, category,
                        day_total, month_total, year_total, total_minutes)
  values (v_year, v_month, v_today, p_category, p_delta, p_delta, p_delta, p_duration)
  on conflict (key_date, category) do update
  ...
  insert into stats_teacher(email, key_year, key_month, key_date, category,
                            day_total, month_total, year_total, total_minutes)
  values (p_email, v_year, v_month, v_today, p_category, p_delta, p_delta, p_delta, p_duration)
  on conflict (email, key_date, category) do update
